...
fun NewAppointmentScreen(
    ...
    var visitType by remember { mutableStateOf(VisitType.NEW) }
    var appointmentDate by remember { mutableStateOf<Date?>(null) }
    var appointmentTime by remember { mutableStateOf<Date?>(null) }
    ...
    val addAppointmentState by viewModel.addAppointmentState.collectAsState()

    HandleUiStatesAppointment(addAppointmentState, context, viewModel, onAppointmentAdded)
    ...
    Column(
            ...
            onClick = {
                coroutineScope.launch {
                    ...
                    if (!dateError && !timeError) {
                        val appointment = PatientAppointment(
                            date = appointmentDate,
                            time = appointmentTime,
                            visitType = visitType,
                            notes = notes.ifEmpty { null }
                        )
                        viewModel.addAppointment(patientId, appointment)
                    }
                }
            },
        ...
        if (addAppointmentState is UiState.Loading) {
...
fun HandleUiStatesAppointment(
    ...
    when (state) {
        is UiState.Success -> {
            LaunchedEffect(Unit) {
                onAppointmentAdded()
                viewModel.resetAddAppointmentState()
            }
        }
