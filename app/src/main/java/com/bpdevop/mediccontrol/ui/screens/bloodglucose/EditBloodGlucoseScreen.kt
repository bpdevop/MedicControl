...
@Composable
fun EditBloodGlucoseScreen(
    patientId: String,
    bloodGlucose: BloodGlucose,
    viewModel: BloodGlucoseViewModel = hiltViewModel(),
    onBloodGlucoseUpdated: () -> Unit,
) {
    val context = LocalContext.current
    val coroutineScope = rememberCoroutineScope()

    var glucoseType by remember { mutableStateOf(bloodGlucose.type) }
    var glucoseResult by remember { mutableIntStateOf(bloodGlucose.result.toInt()) }
    var glucoseDate by remember { mutableStateOf(bloodGlucose.date) }
    var glucoseTime by remember { mutableStateOf(bloodGlucose.time) }
    var notes by remember { mutableStateOf(bloodGlucose.notes ?: "") }
    var showDatePicker by remember { mutableStateOf(false) }
    var showTimePicker by remember { mutableStateOf(false) }
    var unit by remember { mutableStateOf(bloodGlucose.unit) }

    var glucoseDateError by remember { mutableStateOf(false) }
    var glucoseTimeError by remember { mutableStateOf(false) }

    val editGlucoseState by viewModel.updateBloodGlucoseState.collectAsState()

    // Manejar el estado de edición
    when (editGlucoseState) {
        is UiState.Success -> {
            LaunchedEffect(Unit) {
                onBloodGlucoseUpdated()
                viewModel.resetUpdateBloodGlucoseState()
            }
        }

        is UiState.Error -> {
            LaunchedEffect(Unit) {
                Toast.makeText(context, (editGlucoseState as UiState.Error).message, Toast.LENGTH_LONG).show()
                viewModel.resetUpdateBloodGlucoseState()
            }
        }
    ...
    Column(
            ...
            onClick = {
                coroutineScope.launch {
                    ...
                    if (!glucoseDateError && !glucoseTimeError) {
                        val updatedBloodGlucose = bloodGlucose.copy(
                            type = glucoseType,
                        ...
                        viewModel.updateBloodGlucose(patientId, updatedBloodGlucose)
                    }
                }
            },
        ...
        if (editGlucoseState is UiState.Loading) {
